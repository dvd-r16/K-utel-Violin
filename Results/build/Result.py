...
def generar_graficas_resultado():
    ...
    try:
        ...
        df_leccion = df[df['leccion'] == 1].sort_values(by='intento', ascending=False).head(10).sort_values(by='intento')
...
def actualizar_nivel_usuario_leccion1(porcentaje):
    try:
        if porcentaje < 80:
            return  # no aplica
            ...
            if linea.startswith("Nivel:"):
                ...
                if nivel_actual == 0:
                    ...
                    with open(user_file, 'w', encoding='utf-8') as f:
                        f.writelines(lineas)
                    print("[INFO] Nivel actualizado a 1.")
                break
    except Exception as e:
        print(f"[ERROR] Al actualizar nivel desde lección 1: {e}")


def cargar_estrellas_leccion1():
    try:
        with open(USER_SELECTED_PATH, 'r') as f:
            user_id = f.read().strip()
        ...
        df_leccion1 = df[df['leccion'].str.contains("Lección 1", na=False)]

        if df_leccion1.empty:
            print("[INFO] No hay registros en Lección 1")
            return 0, 0  # porcentaje, valor

        fila_ultima = df_leccion1.sort_values(by="intento", ascending=False).iloc[0]
        valor = fila_ultima["valor"]
        porcentaje = int(round(valor / 10 * 100))
        print(f"[INFO] Último puntaje L1: {valor} → {porcentaje}%")
        return porcentaje, valor
    except Exception as e:
        print(f"[ERROR] Al cargar estrellas L1: {e}")
        return 0, 0
...
porcentaje, valor = cargar_estrellas_leccion1()
estrellas_visibles = porcentaje // 20
...
actualizar_nivel_usuario_leccion1(porcentaje)
