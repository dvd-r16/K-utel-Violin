...
fase_exploracion = True  # Modo sin evaluación hasta tick 40
tick_total = 0
                ...
                if (shoulder[2] > args.detection_threshold and
                    ...
                    if evaluar_tick:
                        ...
                        if not fase_exploracion:
                            ...
                            print(f"[EVAL] {estado_eval} | Evaluación #{evaluaciones_realizadas}/10")

                        if evaluaciones_realizadas >= 10:
                            registrar_resultado(leccion_idx=0, aciertos=resultados.count(True))  # Lección 2
                            distribuir_puntos_en_txt(leccion_idx=0, aciertos=resultados.count(True))
            ...
            if b"tick" in data:
                ...
                if tick_count >= 4:
                    ...
                    def liberar_tick_si_no_evaluado():
                        time.sleep(2)
                        if evaluar_tick:
                            print("[WARN] Evaluación no procesada a tiempo. Liberando.")
                            globals()["evaluar_tick"] = False

        threading.Thread(target=liberar_tick_si_no_evaluado, daemon=True).start()
    except Exception as e:
...
def registrar_resultado(leccion_idx, aciertos):
    ...
    try:
        ...
        valor = round((sum(resultados) / 10) * 10, 2)
        intento = 1
        if csv_file.exists():
            with open(csv_file, 'r') as f:
                reader = list(csv.DictReader(f))
                if reader:
                    intentos_filtrados = [int(r["intento"]) for r in reader if r["leccion"] == f"Lección {leccion_idx+1}"]
                    if intentos_filtrados:
                        intento = max(intentos_filtrados) + 1
        ...
        print(f"[CSV] Resultado registrado: Lección {leccion_idx}, Valor: {valor}")
    except Exception as e:
