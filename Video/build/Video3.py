...
VIDEO_PATH = BASE_PATH / "Video" / "build" / "assets" / "frame0" / "video03.mp4"
ANALISIS_PATH = BASE_PATH / "Camara" / "iniciar_analisis3.py"
...
def omitir_intro(event=None):
    ...
    if video_omitido:
        return
    video_omitido = True
    print("[EVENTO] Intro Lección 3 omitida.")
    if vlc_player:
        vlc_player.stop()
        vlc_player.release()
    canvas.delete("all")
...
def on_video_final(event=None):
    print("[EVENTO] Video Lección 3 finalizado.")
...
def reproducir_video():
    ...
    vlc_player.set_xwindow(canvas.winfo_id())  # Linux (X11)
