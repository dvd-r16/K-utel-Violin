...
ANALISIS_PATH = BASE_PATH / "Camara" / "iniciar_analisis2.py"
...
def omitir_intro(event=None):
    ...
    if video_omitido:
        return
    video_omitido = True
    print("[EVENTO] Intro lección 2 omitida.")
    if vlc_player:
        vlc_player.stop()
        vlc_player.release()
    canvas.delete("all")
...
def on_video_final(event=None):
    print("[EVENTO] Video lección 2 finalizado.")
